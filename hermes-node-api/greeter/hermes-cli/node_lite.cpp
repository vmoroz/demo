...
/*static*/ void NodeLiteRuntime::Run(std::vector<std::string> argv) {
  ...
  // }
  // if (!fs::exists(js_root)) {
  //   NodeLiteErrorHandler::ExitWithMessage("Error: Cannot find test
  //   directory.");
  // }

  fs::path js_root = fs::current_path();
...
void NodeLiteRuntime::DefineBuiltInModules() {
  ...
  // Define "bindings" module
  {
    node_js_modules_.try_emplace("bindings", "bindings");
    AddNativeModule("bindings", [this](napi_env env, napi_value exports) {
      return NodeApi::CreateFunction(
          env_, "bindings", [this](napi_env env, span<napi_value> args) {
            NODE_LITE_ASSERT(args.size() == 1,
                             "Expected 1 argument, but got: %zu",
                             args.size());
            std::string native_module = NodeApi::ToStdString(env, args[0]);
            fs::path module_path = fs::path(js_root_) / "build" / "Release" /
                                   (native_module + ".node");
            if (!fs::exists(module_path)) {
              module_path =
                  fs::path(js_root_) / "build" / (native_module + ".node");
            }
            if (!fs::exists(module_path)) {
              return NodeApi::GetUndefined(env);
            }
            ...
            NodeLiteRuntime* runtime = NodeLiteRuntime::GetRuntime(env);
            return runtime->ResolveModule(js_root_, module_path.string())
                .LoadModule(env);
          });
      return exports;
    });
